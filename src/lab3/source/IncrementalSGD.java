...
public class IncrementalSGD {
	...
	public static void main(String args[]) throws Exception {
		...
		while ((current = loader.getNextInstance(dataStructure)) != null) {// System.out.println("instance
																			// "
																			// +
																			// current);
		...
		inMemoryData.setClassIndex(inMemoryData.numAttributes() - 1); // for the
																		// data
																		// read
		...
		Evaluation eTest = new Evaluation(inMemoryData); // cross validate
															// inMemory
															// instances
		...
		/*
		 * for (int i = 0; i < dataT.numInstances(); i++) { double pred =
		 * sgd.classifyInstance(dataT.instance(i)); System.out.print("Instance "
		 * + i); if(dataT.instance(i).isMissing(dataT.numAttributes() - 1) )
		 * System.out.print(", actual: ?"); else System.out.print(", actual: " +
		 * dataT.classAttribute().value((int) dataT.instance(i).classValue()));
		 * System.out.print(", predicted: " + dataT.classAttribute().value((int)
		 * pred)); System.out.println(" with p="+Double.toString(pred));
		 * 
		 * }
		 */
